...
if [[ -z "$HYTALE_SERVER_SESSION_TOKEN" ]]; then
    ...
    HYTALE_LATEST_VER=$(./hytale-downloader/hytale-downloader-linux -print-version)
    
    # Read the currently installed version if the file exists
    if [[ -f "hytale-current-ver.txt" ]]; then
        INSTALLED_VER=$(cat hytale-current-ver.txt)
    else
        INSTALLED_VER="none"
    fi

    # Compare versions: Download if they don't match
    if [[ "$HYTALE_LATEST_VER" != "$INSTALLED_VER" ]]; then
        echo "Update required. Installed: $INSTALLED_VER, Latest: $HYTALE_LATEST_VER. Downloading..."
        ...
        # Save the new version to the tracking file
        echo "$HYTALE_LATEST_VER" > hytale-current-ver.txt
    else
        echo "Server is already up to date ($INSTALLED_VER)."
    fi
